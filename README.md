## DP 초기화 시 인덱스 오류 개선
DP 문제를 풀 때,  입력값에 따라 arr을 선언하면 다음과 같이 오류가 날 수 있다.
`arr[2]`에 접근하는 코드가 있는데 `n=1`이면 `IndexError`가 발생한다.

이를 방지하기 위해, DP 배열을 선언할 때  
**최소 크기를 보장하는 방식으로 초기화**하면 된다.
...
# 최소 크기를 3으로 설정해 n이 작아도 안전하게 접근 가능
